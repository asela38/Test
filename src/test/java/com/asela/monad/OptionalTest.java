...
public class OptionalTest {
    ...
    public static Optional<Person> createPerson(FieldProvider<String> fieldProvider) {
        return fieldProvider.getField().flatMap(name -> fieldProvider.getField().flatMap(line1 -> fieldProvider
                .getField().flatMap(line2 -> fieldProvider.getField().flatMap(cityName -> fieldProvider.getField()
                        .map(cityCode -> new Person(name, new Address(line1, line2, new City(cityName, cityCode))))))));
    }

    public static void print(Person person) {
        person.getName().map(name -> "N: " + name + person.getAddress()
                .map(address -> ", address -> " + address.getLine1().map(line1 -> "L1: " + line1 + address.getLine2()
                        .map(line2 -> ", L2: " + line2 + address.getCity().map(city -> ", city -> " + city.getName()
                                .map(cityName -> " CN: " + cityName
                                        + city.getCode().map(cityCode -> ", CC: " + cityCode + "").orElse(" <NULL>"))
    ...
    public void testSomeWiredStuff() {
        ...
        Optional.of(people.get("AA")).flatMap(Person::getAddress).flatMap(Address::getCity).flatMap(City::getCode)
        ...
        Optional.of(people.get("BB")).flatMap(Person::getAddress).flatMap(Address::getCity).flatMap(City::getCode)
                .ifPresent(System.out::println);
    }
    ...
    public void testNameee() throws Exception {
        ...
        System.out.println(encodePassword2("Hello"));
    }
    ...
    public Optional<String> concat(Optional<String> one, Optional<String> two) {
        return one.flatMap(o -> two.flatMap(t -> Optional.of(o.concat(t))));
    }
